function [eigenVals, weights] = getEigenVal_Xc_incorporated(eigen_guesses, x_vec, prevPop, ...
...
%% picards iteration for each guess 1 time 
    ...
    %% rayleigh iteration  on each guess 
    for g = 1:numGuesses
        ...
        for iter = 1:MaxIter
            ...
            Xc = [res; 1e2* x_bc;1e1* X_g ; 1e3* X_ortho];
            Yc = [zeros(m2, 1); 1e2*y_bc; 1e1; 1e3*Y_ortho];
            %  optimal solution 
