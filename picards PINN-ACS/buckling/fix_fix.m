...
X = [A - eigenVal*B; 1e6*x_bc;1e2* Xref];         
Y = [zeros(m2,1);1e6*y_bc;1e2* Yref]; 
% inital guess 
w2_g = pinv(X)*Y; 
...
for iter = 1:MaxIter  
    ...
    res = A - (eigenVal) * B;
    coeff = (1-0)/numel(x_vec); % monte carlo approximation coefficient
    X_p = cos(w1*x_vec + b1 )';
    X_p = coeff.*((u_g') * X_p);
    %  combining all the residuals (pde bc , normalisaiton)
    X = [res; 1e6*x_bc;1e2* X_p];         
    Y = [zeros(m2,1);1e6*y_bc;1e2* 1];  
    w2 = pinv(X)*Y;
...
function printEigenMode(w2 , w1 ,b1,L , pcric)
    ...
    figure;
    plot(x_vec ,pred , 'b');
    title(['eigen Function at critical Load (fix-fix) ',num2str(pcric)]);
    xlabel('x');
