...
elapsed_time =toc;
fprintf("crtical buckling load (fix-free(transl free and rotational fix)) %.6f\n " ,eigenVal);
...
function printEigenMode(w2 , w1 ,b1,L , pcric)
    ...
    figure;
    plot(x_vec ,pred , 'b');
    title(['eigen Function at critical Load (fix- free (transl free and rotation fix)) ',num2str(pcric)]);
    xlabel('x');
