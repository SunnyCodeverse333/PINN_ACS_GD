...
% === Boundary Conditions for Fixed-pinned Beam ===
x_bc = zeros(4, n);
y_bc = zeros(4, 1);

x_bc(1, :) = cos(w1 * 0 + b1)';                              % w(0)
x_bc(2, :) = cos(w1 * L + b1)';                              % w(L)
x_bc(3, :) = -sin(w1 * 0 + b1)' .* w1';                      % w'(0)
x_bc(4, :) = -cos(w1 * L + b1)' .* (w1 .^ 2)';               % w''(L)
...
for iter = 1:MaxIter  
    ...
    eigenVal =abs(newApproxEigen);
    w2Opt = w2;
end
elapsed_time =toc;
fprintf("crtical buckling load (fix-pinned ends) %.6f\n " ,eigenVal);
...
function printEigenMode(w2 , w1 ,b1,L , pcric)
    ...
    figure;
    plot(x_vec ,pred , 'b');
    title(['eigen Function at critical Load (fix-pinned) ',num2str(pcric)]);
    xlabel('x');
